...
def test_valid_factory_config_parses() -> None:
    ...
    assert config.agents[0].sandbox == "read-only"
...
def test_agent_sandbox_must_be_known(tmp_path: Path) -> None:
    path = tmp_path / "factory.yaml"
    path.write_text(
        """
factory:
...
""",
    ...
    with pytest.raises(ConfigError, match="agents.planner.sandbox must be read-only or danger-full-access"):
        load_factory_config(path)
